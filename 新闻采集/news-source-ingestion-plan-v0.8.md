...
- 新增跨模块可复用的业务任务审计账本；通用核心不依赖新闻模型，新闻采集只是首个适配模块。
- 状态变更或产物命令统一记录任务五要素、实际阶段、漏斗、执行状态与设计状态；只读、dry-run 与数据库迁移明确排除。
- 完整 `run` 固定为八阶段，自动校验顺序、前置与同单位守恒；已发生偏差强制退出码 9，不得显示成功。
- 新增 `task list/show` 人读与 JSON 下钻；详情通过任务/阶段关联，长期账本不复制原始响应和正文。
- 增加 cron 触发上下文、非标准补跑原因、30 分钟 stale 恢复、0005 迁移及可签署上线/回滚附录。
...
审计层面向新闻采集及后续获准工程化的其它模块，不是采集定制日志。通用核心只认识工作流、任务、阶段、
漏斗和校验结论；领域适配器负责声明流程、提取受控统计和解析详情。

每个受控写操作先持久化 `business_task`，记录“谁、何时、对什么、做了什么、结果怎样”。阶段事实写入
`business_task_stage`，以数据库内 `actual_sequence` 为实际顺序，不能用时间戳猜顺序。完整 `run` 的
唯一顺序为 `fetch → metadata_dedup → classify → content → content_dedup → cluster → score → safety`。
业务服务仍用短事务；业务提交后才能终结阶段。顺序、前置或守恒偏差会停止后续阶段，结果固定为
`partial_success/deviation/9`，不自动重跑或推断修复。
...
一级识别每次调用仍完整记录在 `llm_run`；`news_article.relevance_*` 只物化最新一次 Schema 校验成功的结果，失败降级时物化 `uncertain`。Prompt 升版后，`classify --stale --reason "原因"` 重跑旧版本结果；新的成功结果可以把此前 `irrelevant` 的文章改为 `relevant/uncertain` 并重新进入正文抓取，历史 `llm_run` 不覆盖。
...
`business_task` 长期保留触发者、操作、范围、预期工作流、执行/设计状态、退出码、摘要和校验结论。
执行状态为 `requested | running | succeeded | partial_success | failed | blocked | abandoned`；设计状态为
`pending | compliant | deviation | incomplete`。被前置检查阻止表示保护生效，不是已发生的设计偏差。

`business_task_stage` 记录预期/实际序号、尝试次数、单位、输入/输出、分流、原因、受控指标和校验快照。
不同单位只记录显式转换，不机械守恒。`fetch_log`、`llm_run` 通过可空的任务/阶段复合键关联：历史记录可双空，
新审计上下文必须双非空，禁止半关联或跨任务拼接。账本不复制正文、`raw_response`、长错误或凭据。

### 9.11 留存边界（v0.8）

任务、阶段、工作流快照、受控摘要和校验结论长期保留，本版不自动删除。`content_raw` 保留 7 天；
`content_clean`、LLM 原始响应/脱敏错误和文件技术日志保留 30 天。详情到期不破坏长期设计验证。
...
$env:NEWS_TRIGGER_TYPE='cron'
$env:NEWS_OPERATOR='scheduler:daily'
$env:NEWS_AUDIT_REASON='人工确认的数据修复原因'
```

`trigger_type` 默认 manual，`operator` 默认当前系统用户。非标准补跑必须通过全局/命令 `--reason`
或 `NEWS_AUDIT_REASON` 提供非空原因；cron 不得依赖交互输入。

| 分类 | operation | 任务规则 |
|---|---|---|
| 标准写任务 | `run`、`fetch`、`event.review`、`event.fact-check`、`export`、`supabase.sync` | 每次调用恰好一个 standard 任务 |
| 非标准补跑 | `dedup`、`cluster`、`classify`、`score`、`llm.retry`、`article.refetch` | `path_type=non_standard`，写入前检查前置并要求原因 |
| 运维写任务 | `retention.prune`、`pool-index` | 记录 operations 任务；dry-run 除外 |
| 排除 | `db upgrade/status`、`source list/validate`、`event list`、`fetch-log`、`health`、`task list/show`、`retention prune --dry-run` | 只读或基础设施动作，不创建任务 |
...
# 重新抓取某篇文章（非标准补跑）
uv run news-ingestion article refetch <article-id> --reason "人工确认的原因"

# 执行去重
uv run news-ingestion dedup --since 24h --reason "人工确认的原因"

# 执行事件聚类
uv run news-ingestion cluster --since 72h --reason "人工确认的原因"

# 执行LLM轻量识别
uv run news-ingestion classify --since 24h --reason "人工确认的原因"

# 执行LLM完整评分
uv run news-ingestion score --event <event-id> --reason "人工确认的原因"

# 重试LLM失败任务
uv run news-ingestion llm retry --status failed --reason "人工确认的原因"

# 人工修正或确认事件进入合格新闻池
uv run news-ingestion event review <event-id> --approve [--reviewer <name>]

# 查看待复核事件
uv run news-ingestion event list --review-status pending

# 记录事件的事实核验结论与证据
uv run news-ingestion event fact-check <event-id> --status verified [--reviewer <name>]
...
# 主流水和五要素/阶段/漏斗/详情下钻（只读，不建任务）
uv run news-ingestion task list
uv run news-ingestion task list --status deviation --module news_ingestion --since 2026-07-15T00:00:00+08:00 --json
uv run news-ingestion task show <task-id>
...
`run` 是日常默认入口，严格按 7.1 的八阶段执行，完成后停在人工复核队列。单阶段命令只用于有原因的诊断和补跑。`run` 不得自动批准、自动导出或调用下游流程；没有配置 LLM 凭据时，一级按 `uncertain` 降级并继续，二级保持 `pending`。
...
每个来源使用独立数据库事务。来源成功时文章变更和 `fetch_log=success|partial_success` 一起提交；来源失败时回滚该来源的文章变更，并单独写入脱敏后的 `fetch_log=failed`。进程异常退出后，下次任务取得同一锁域后，把超过 30 分钟的旧 requested/running 任务收敛为 `abandoned/incomplete/6`，活动阶段标 abandoned；不得推断成功或自动重跑。最终 CLI 摘要列出成功、部分成功、失败、禁用和跳过来源。
...
→ 必要时完成 fact-check
→ 逐事件 review
→ export 单一新闻素材库（年龄档与家长档判定留在下游选题/写稿）
```
...
| `9` | 业务前置未满足，或已发生顺序/守恒设计偏差 |

执行状态为 `requested/running/succeeded/partial_success/failed/blocked/abandoned`；设计状态为
`pending/compliant/deviation/incomplete`。退出 9 时调度器停止自动重试，先用 `task show` 区分
`blocked/compliant` 与 `partial_success/deviation`；需要补偿时创建带原因的新任务。审计终结失败返回 6，
即使业务可能已提交也禁止盲目重跑。任何非零退出不得删除此前已成功提交的数据。
...
- [ ] 任一受控写命令恰好创建一个任务；排除命令不创建任务。
- [ ] 完整 run 可按一个任务 ID 复述五要素、八阶段实际顺序、漏斗与设计结论。
- [ ] 前置不满足在写入前记录 `blocked/compliant/9`；已发生偏差记录 `partial_success/deviation/9` 并停止后续阶段。
- [ ] 无 LLM 为 `partial_success/compliant/7`；部分来源失败为 `partial_success/compliant/4`；正常完成为 `succeeded/compliant/0`。
- [ ] 业务提交后审计终结失败返回 6，不回滚业务、不推断成功、不自动重跑。
- [ ] 非新闻假模块无需修改通用核心即可声明任务、阶段、漏斗和校验。
...
当前迁移链固定为 `0001 → 0002 → 0003 → 0004 → 0005`。0005 创建任务/阶段表，为
`fetch_log/llm_run` 增加双键关联，并在 Postgres 启用 RLS、撤销客户端角色权限。历史详情不虚构回填任务。

每个受控任务先以 requested 单独提交，再取得进程锁与同锁域 advisory lock；持锁后按 15 节恢复 stale。
生产回滚优先旧代码配合保留的 0005 增量 schema。已有审计证据时不得现场 downgrade；只有先备份/导出，
并在隔离副本证明 `0005 → 0004 → 0005` 只影响本次对象且旧业务基线不变，才允许 schema downgrade。

## 21. 可签署上线附录

每步记录时间、环境、操作者、当前/目标 revision、任务 ID、观察信号、结论和失败动作。

### 21.1 发布负责人

- [ ] 暂停 cron，并用锁、进程与任务查询证明没有活跃执行。
- [ ] 收齐数据库、功能和值守负责人签字后作最终 Go/No-Go。
- [ ] 任一 No-Go 出现时保持 cron 暂停，执行恢复或回滚。

### 21.2 数据库负责人

- [ ] 备份并记录旧业务基线与 `db status`；确认目标 head 为 0005。
- [ ] 执行 `uv run news-ingestion db upgrade` 和 `uv run alembic check`。
- [ ] 验证新表、约束、索引、复合外键、RLS/权限及旧业务基线未变化。
- [ ] 在隔离 Postgres 完成空库到 head、0004→0005 和 downgrade/upgrade 演练；生产不做破坏性演练。

### 21.3 功能负责人

- [ ] 执行受控空数据或假数据写任务，确认恰好产生一个合法任务。
- [ ] 用 `task list --json` 与 `task show <id> --json` 验证单一 JSON、五要素、阶段、漏斗和详情关联。
- [ ] 验证只读命令不建任务，非标准补跑无原因在业务写入前返回 9。

### 21.4 值守负责人

- [ ] 检查恢复 cron 后首个任务和前三次调度。
- [ ] 在 +4h、+24h 检查：退出码 6 为零；无超出“最长正常运行时间 + 预留窗口”的非终态；
  无未解释的 deviation/incomplete；新详情可下钻；JSON stdout 可解析。

### 21.5 固定部署顺序与 No-Go

1. 暂停 cron，证明无活跃执行。
2. 备份并确认当前 revision 与旧业务基线。
3. 执行迁移到 0005。
4. 验证 schema、约束、RLS/权限和旧业务基线。
5. 执行受控空/假数据任务。
6. 验证任务唯一、`task list/show` 单一 JSON 与详情下钻。
7. 四类负责人签署后恢复 cron。

No-Go：任一自动门槛失败；revision/schema/RLS 不符；旧业务基线变化；受控写任务不是恰好一个合法任务；
JSON stdout 被污染；审计路径出现退出码 6。No-Go 时不得恢复 cron。
...
live gate 必须使用 Supabase-shaped 临时或隔离 Postgres，且不得等于生产 `SUPABASE_DB_URL`；它验证真实行锁下的
阶段竞争、任务终态 CAS、详情复合外键以及 public/anon/authenticated 拒绝访问。数据库负责人另按 21.2
完成迁移往返与旧业务基线核对。默认离线测试不得连接真实 Supabase、真实来源或真实 Kimi。

## 23. 代码事实源

- 命令覆盖/排除：`src/news_ingestion/cli.py::AUDITED_COMMANDS/EXCLUDED_COMMANDS`
- 通用契约/校验：`src/news_ingestion/audit/contracts.py`、`validation.py`
- 新闻八阶段：`src/news_ingestion/audit/news_ingestion.py::NEWS_INGESTION_WORKFLOW`
- 生命周期/恢复：`src/news_ingestion/services/audit_service.py`、`audited_command.py`
- 状态/迁移：`src/news_ingestion/models/audit_*.py`、`migrations/versions/0005_business_task_audit.py`
- 查询下钻：`src/news_ingestion/services/audit_view.py`
