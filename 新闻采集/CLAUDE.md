...
- `news-source-ingestion-plan-v0.8.md` 是本模块当前唯一实施方案（v0.8：服务器定时一次性任务 + Supabase 唯一运行库 + 通用业务任务审计；采集只硬过滤红线）。数据契约、采集约束、退出码、验收标准、实施顺序**一律以它为准，不得凭记忆改写**。`v0.7.md` 及更早版本已归档。
- 做实质性修订时，同步升版文件名（v0.5…）、文档版本号与日期，并更新本目录 `README.md` 的指针——参照父 CLAUDE.md 的编辑约定。
- 本文件不定义产品边界或安全判定逻辑：产品边界看 `../PRD_v0.9.md`，安全判定看 `../Content_Safety_Policy_v0.1.md`。

## 当前状态（先看这条）

**MVP 与 v0.8 统一业务任务审计已实现，`uv run pytest` 全绿。** 目录已含 `pyproject.toml`、`uv.lock`、`.python-version`（3.12）、`src/news_ingestion/`、`migrations/`、`tests/`、`config/`、`prompts/`。
...
**本模块不负责**：后台常驻或实时抓取服务（允许外部 cron 定时触发一次性任务）；生成 3 候选、标 1 推荐；替家长确认最终选题；**家长档 × 年龄档判定与事实核验**（这些在下游选题/写稿时做）；写稿、TTS、音频评估或发布。人工或家长临时发现的材料**不录入本模块数据库**，直接走下游手工选题。CLI 不调用下游写稿或音频流程，不写 `稿子/` 目录。采集之后仍进入现有手工流程：新闻素材库 → 手工出 3 候选标 1 推荐（此时做家长档×年龄档判定与事实核验）→ 家长拍板 → 写稿 → 音频。

## 技术栈与依赖管理（uv 铁律）

锁定栈：**Python 3.12 + uv + Supabase Postgres（唯一生产 DB）+ psycopg 3 + SQLAlchemy 2.x + Alembic + Typer CLI**（CLI 名 `news-ingestion`）。测试可显式使用临时 SQLite，但生产入口拒绝 SQLite。运行依赖固定 Typer / SQLAlchemy / Alembic / psycopg / Anthropic Python SDK / jsonschema；测试依赖固定 pytest，用 `uv run pytest`。
...
uv run news-ingestion classify --stale --reason "Prompt 升版重评旧 irrelevant"
uv run news-ingestion score --event <id> --reason "人工确认的原因"
uv run news-ingestion llm retry --status failed --reason "人工确认的原因"
uv run news-ingestion event list --review-status pending
uv run news-ingestion event review <id> --approve [--reviewer <name>]
uv run news-ingestion event fact-check <id> --status verified [--reviewer <name>]
uv run news-ingestion export               # v0.5：导出单一新闻素材库（不分年龄/家长档）
uv run news-ingestion supabase sync        # 可选：幂等同步最新成功导出的 JSON 快照
uv run news-ingestion retention prune     # 幂等，Asia/Shanghai，默认实清，提供 --dry-run
uv run news-ingestion task list           # 只读主流水；支持 --json/--status/--module/时间范围
uv run news-ingestion task show <task-id> # 只读五要素、阶段、漏斗、偏差和详情下钻
...
- 审计核心必须保持跨模块通用，不得导入新闻模型；新闻采集通过适配器声明八阶段和详情解析。
- 受控写命令每次恰好一个任务；只读、`db upgrade/status` 和 `retention prune --dry-run` 不建任务。
- `dedup/cluster/classify/score/llm retry/article refetch` 属非标准补跑，必须提供 `--reason` 或 `NEWS_AUDIT_REASON`。
- 执行状态与设计状态分开；被阻止是 `blocked/compliant`，已发生偏差是 `partial_success/deviation/9`。
- 退出 9 或审计/数据库退出 6 时禁止调度器盲目重跑；先 `task show`，补偿必须新建带原因任务。
- 超过 30 分钟的旧非终态只能在取得同锁域后恢复为 `abandoned/incomplete/6`，不得推断成功。
- 任务/阶段及受控快照长期保留；正文、LLM 原始响应/错误和文件日志按 7/30 天规则清理。

### 凭据规则

`SUPABASE_DB_URL` 必须使用 Supabase Connect 提供的 Direct 或 Session pooler PostgreSQL 连接串（包含数据库密码，不是 API key）。`ANTHROPIC_BASE_URL` 与 `ANTHROPIC_API_KEY` 由服务器密钥环境注入（另有 `RSSHUB_BASE_URL` 支持自建 RSSHub）；`ANTHROPIC_BASE_URL` 规范值为 `https://api.kimi.com/coding/`。**密钥不许写入**配置、日志、异常、数据库或仓库；持久化 `llm_run` 前必须移除 API Key、Authorization/x-api-key 头、Cookie，错误文本设长度上限。

## 架构与数据流

**唯一权威执行顺序**（plan §7）：源库 → 服务器定时触发一次性 CLI → 采集 → 原始文章库 → URL/标题去重与规则预筛 → 一级 LLM → 正文抓取与内容去重 → 事件聚类 → 二级 LLM → JSON Schema 校验 + 安全兜底 → 人工复核 → 导出。`fetch` 只发现并写元数据；`run` 严格按图推进，**不许先聚类再做一级识别**。
...
**数据模型**（plan §9）：Supabase 八张运行表是唯一采集状态事实源（原七张表 + `cluster_forbid_pair`）；另有 `news_material_sync_runs`、`news_material_events` 与 `latest_news_material_events` 视图保存版本化导出快照。远端表必须启用 RLS 并撤销 `public` / `anon` / `authenticated` 权限。
...
4. **Prompt 版本化**：Prompt 独立成文件、带版本，不硬编码进业务代码；每次调用记 model/prompt/schema 版本、input_hash、token 与成本。Prompt 升版后用 `classify --stale --reason "原因"` 重评旧 `irrelevant`，历史 `llm_run` 不覆盖。
...
8. **服务器定时、任务一次性退出**：允许 cron 等外部调度器定时触发；不引入后台常驻或实时抓取服务。Postgres advisory lock 防止多实例重叠。
9. **留存**：`content_raw` 7 天、`content_clean` 30 天、文件运行日志与 LLM `raw_response`/脱敏错误 30 天；到期由人工执行 `retention prune`（幂等、Asia/Shanghai、提供 `--dry-run`）清空，**不删文章/事件/来源/hash/事件关联/结构化 LLM 结果/事实核验/复核记录**。采集进程不内置调度器；服务器可用外部 cron 触发一次性命令。
...
- `../PRD_v0.9.md`：产品边界与引擎五层的真理源；本模块的「做什么」由它定义（新闻采集 = Content Intelligence，唯一工程化）。
