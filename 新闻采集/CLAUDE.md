...
- `news-source-ingestion-plan-v0.7.md` 是本模块当前唯一实施方案（v0.7：服务器定时一次性任务 + Supabase 唯一运行库；采集只硬过滤红线）。数据契约、采集约束、退出码、验收标准、实施顺序**一律以它为准，不得凭记忆改写**。`v0.6.md` 已归档。
...
uv run news-ingestion run --json          # 服务器定时任务入口；stdout 输出机器可读结果；未配置 LLM 凭据返回退出码 7，非 LLM 阶段数据保留
