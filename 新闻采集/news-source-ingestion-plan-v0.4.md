...
11. 以 JSON 和 Markdown 导出合格新闻池，为后续手工选题提供标准输入。
...
本模块是根目录 `新闻采集/` 下的本地独立 Python 项目：

- Python 3.12，统一使用 uv 管理 Python、虚拟环境与依赖；`pyproject.toml`、`uv.lock`、`.python-version` 必须提交；
- SQLite 为 MVP 唯一数据库，SQLAlchemy 2.x + Alembic 管理模型和迁移；
- Typer 提供 `news-ingestion` CLI；
- 运行依赖固定为 Typer、SQLAlchemy、Alembic、Anthropic Python SDK、jsonschema；测试依赖固定为 pytest；所有版本由 `uv.lock` 锁定；
- 每次运行由人手动发起，命令完成后退出，不包含常驻服务和调度器；
- 数据库、日志和导出文件默认保存在模块自己的 `data/`、`logs/`、`output/`；
- LLM 固定使用 Kimi Coding 的 Anthropic 兼容接口与 `kimi-for-coding` 模型；程序读取 `ANTHROPIC_BASE_URL`、`ANTHROPIC_API_KEY`，不把密钥写入仓库；
- 单次命令以进程锁防止同一工作区重复运行，失败返回非零退出码并保留已成功提交的来源结果；
...
上图是唯一权威执行顺序。`fetch` 只完成来源发现和文章元数据写入；`run` 必须严格按图推进，不得先聚类再做一级识别。
...
正文留存规则：`content_raw` 自 `fetched_at` 起保留 7 天，`content_clean` 保留 30 天；到期后清空正文列，但长期保留标题、摘要、URL、来源、hash、事件关联和其它元数据。留存清理由人工执行 CLI，不引入调度器。
...
当 `needs_fact_check=true` 时，事件默认核验状态为 `pending`。只有人工填写明确结论，并记录至少一个正式媒体事实源或原始来源 URL 后，才能转为 `verified`。`pending` 或 `failed` 事件不得批准、不得进入合格新闻池。热点雷达来源始终设置 `needs_fact_check=true`。`checker` 必须取自 CLI `--reviewer`，未传时读取 `NEWS_REVIEWER`；两者都缺失或空白时拒绝写入。
...
`reviewer` 必须取自 CLI `--reviewer`，未传时读取 `NEWS_REVIEWER`；两者都缺失或空白时拒绝写入。`eligible` 不是 `news_event.status`，也不由分数或规则自动产生；它只存在于最新人工复核记录的六组合矩阵中。复核人将事件设为 `approved` 时，必须同时填写六个“年龄档 × 家长档”组合的 `eligible / ineligible`；LLM 分数只提供判断材料。二级 LLM 失败时，复核人可通过 `age_assessments_override` 和其它 override 补齐完整评分结果。若 `needs_fact_check=true`，系统必须先验证最新 `fact_check_record.status=verified`，否则拒绝批准。批准前还必须把数据库原值与人工 override 合并成有效事件，并通过 `news-pool/v1` Schema 校验；标题、摘要、儿童入口、安全理由、对应年龄档评估、来源名称/角色/URL 等导出必填字段不得为空字符串、`null` 或占位语。缺失字段由复核 CLI 明确列出，补齐前拒绝批准。设为 `rejected` 时六个组合全部视为 `ineligible`。安全红线不能被人工放宽；`safety_override` 只允许把事件判得更严格，或在补充证据后解除 `uncertain`。每次修改保留新记录，不覆盖历史复核。

### 9.8 合格新闻池导出契约

每次导出必须同时、原子化生成同名 JSON 与 Markdown，不提供单格式导出选项。文件名固定为 `YYYYMMDD_HHmmss_<age_band>_<parent_mode>.json` 和同名 `.md`，时间使用 `Asia/Shanghai`，编码为 UTF-8，不覆盖任何历史文件。先写入同目录临时文件，两种格式都通过校验后再一起改为最终文件名；任一失败则删除本次临时文件。JSON 顶层结构固定为：

```yaml
schema_version: news-pool/v1
generated_at: datetime
age_band: upper_primary | junior_high
parent_mode: conservative | standard | open
...
导出只包含最新人工复核记录为 `approved`，且对应年龄档与家长档组合为 `eligible` 的事件。若 `needs_fact_check=true`，最新事实核验状态还必须为 `verified`。CLI 通过 `--parent-mode conservative|standard|open` 指定家长档，未传时默认 `standard`。红线和 `uncertain` 永不进入自动导出；Markdown 是同一 JSON 的人读视图，不增加独有字段。导出前再次校验整个文件；任何事件不符合 Schema 时整次导出失败，不覆盖上一份成功文件。导出文件不得写入 `稿子/`，下游手工流程自行读取。

当指定组合没有任何合格事件时，仍生成合法的一对 JSON/Markdown：`result: empty`、`events: []`，Markdown 明确写“本次无合格事件”。该情况退出码为 `0`，不得放宽筛选或补凑新闻。
...
本模块直接执行 `../Content_Safety_Policy_v0.1.md`，判定恒为“家长档 × 年龄档”，并分别输出小学高年级和初中结果：

- `redline`：永不进入合格新闻池；
- `sensitive`：按家长档 × 年龄档决定可进入、需提示或不进入；
- `default`：可进入，只做年龄适配；
- `uncertain`：不得自动放行，交人工复核。

关键词只用于召回风险，不能单独完成最终安全分类。同一事件在小学高年级与初中档可以有不同的 `age_eligibility`。
...
MVP数据库锁定 SQLite。模型避免依赖 PostgreSQL 专属数组类型，数组与结构化结果以 JSON 字段保存；数据库文件默认位于 `data/news_ingestion.sqlite3`。
...
# 导出两个年龄档的合格新闻池；每次同时生成 JSON 与 Markdown
...
`--parent-mode` 可选 `conservative`、`standard`、`open`，默认值为 `standard`。
...
数据库初始化和升级只能通过 `uv run news-ingestion db upgrade` 显式执行。其它命令启动时检查 Alembic revision；数据库未初始化或落后于 `head` 时返回退出码 `6`，不得自动迁移。SQLite 连接必须启用 foreign keys、WAL 和配置化 busy timeout。
...
模块完成后由人每天手动运行，至少连续观察两周，再决定是否扩大或替换来源。
