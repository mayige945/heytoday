...
- SQLite 为采集运行的唯一事务数据库，SQLAlchemy 2.x + Alembic 管理模型和迁移；Supabase
  仅保存已导出的新闻素材快照，不反向参与采集状态机；
...
`success`；失败标记 `failed`，最新视图只读取最新成功快照。SQLite 与本地双格式导出仍是
运行事实源，Supabase 故障不得回滚或删除本地产物。
...
采集运行数据库锁定 SQLite。模型避免依赖 PostgreSQL 专属数组类型，数组与结构化结果以 JSON 字段保存；数据库文件默认位于 `data/news_ingestion.sqlite3`。Supabase 只承载导出后的版本化素材快照，其 DDL 由 `sql/supabase_news_material_schema.sql` 管理。
...
Supabase 同步要求运行环境提供 `SUPABASE_URL` 与 `SUPABASE_SECRET_KEY`；旧项目可临时使用
