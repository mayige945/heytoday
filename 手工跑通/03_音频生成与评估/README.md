...
6. 用 ASR 对齐原稿，抓错读、漏读、专名问题；多音字必须对照 request JSON，确认不是只在文档里写了、实际 payload 没带。
7. 按 `音频自然度与情绪评估工具调研_v0.1.md` 做自动预检：ASR 对齐、情绪/语气识别、自然度 MOS 或可用替代指标，输出高风险 turn 排序。
8. 根据自动预检结果生成小修建议：错读/漏读、专名多音字、拼接突兀、情绪不一致、自然度低分别标原因；建议只能指向音频导演层、发音词典、停顿、标签或局部重生成。
9. 成人抽听高风险 turn、开头、结尾和少量随机 turn；自动分数只负责路由复听，不单独决定通过。
10. 小修最多 2-3 轮，仍不过线就退回 `02_音频导演稿/`。
...
- `音频/xxx.eval_summary.md`（可选；汇总自动预检、成人复听和小修结论）
...
- 情绪峰值和导演意图大体一致。
- 自动评估已产出高风险 turn 排序；成人复听已覆盖高风险 turn、开头、结尾和随机 turn。
